...
class ComputeLoss:
    ...
    该函数输出的正样本框比传入的targets(GT框也就是标注的物体的先验框)数目多
    具体处理过程：
    (1)对于任何一层计算当前bbox和当前层anchor的匹配程度，不采用iou，而是shape比例；
       如果anchors和bbox的宽高比差距大于4，则认为不匹配，此时忽略相应的bbox，即当作背景
    (2)然后对bbox计算落在网格所有anchors都计算loss(并不是直接和GT框比较计算loss)
       注意此时落在网格不再是一个，而是附近多个，这样就增加了正样本数，可能存在有些bbox在三个尺度都预测的情况；
    ...
    def build_targets(self, p, targets):
        ...
        targets：GT框; targets.shape = (nt, 6), 6=icxywh,i表示第i+1张图片,c为类别,然后xywh为坐标
        self应该是模型
        """
        # Build targets for compute_loss(), input targets(image,class,x,y,w,h)
        # targets nx6,其中n是batch内所有的图片label拼接而成
        # 6的第0维度表示当前是第几张图片的label = index，后面是classid xywh
        na, nt = self.na, targets.shape[0]  # number of anchors, targets    anchor数量(na)和标签框数量(nt)(因为是一个batch总的gt框的数目，且经过mosiac增强后的，所以标签框多了)
        ...
        # 先repeat(重复) targets和当前层anchor个数(当前层anchor数为3)一样,相当于每个bbox变成了三个,然后和3个anchor单独匹配
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        # 设置网格中心偏移量
        g = 0.5  # bias
        # 附近的四个网格
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m 只找了正上，正下，正左，正右
                            # [1, 1], [1, -1], [-1, 1], [-1, -1],  # jk,jm,lk,lm 没找左上，左下，右上右下
                            ], device=targets.device).float() * g  # offsets
        # 对每个检测层进行处理
        for i in range(self.nl):    # 三个尺度的预测特征图输出分支 第一次80*80 第二次40*40 第三次20*20 共三个尺度
            ...
            # 然后将target wh shape和anchor的wh计算比例，如果比例过大，说明匹配度不高，将该bbox过滤，在当前层认为是bg
            if nt:
                # Matches
                """
                ...
                """
                # 计算当前target的wh和anchor的wh比值
                # 如果比值大于预设值model.hyp['anchor_t']=4，则当前target和anchor匹配度不高，不强制回归，而把target丢弃
                # 计算比值(ratio)
                ...
                在选取gij(也就是标签框分配给的网格)的时候对这2个部分的框都做一个偏移(减去上面的offsets),也就是下面的gij = (gxy - offsets).long()操作;
                ...
                t = t.repeat((5, 1, 1))[j]      # 5的原因是预设的off是5个
                # 添加偏移量
                offsets = (torch.zeros_like(gxy)[None] + off[:, None])[j]   # 选出最近的三个(包括自己)
            else:
                t = targets[0]
                offsets = 0
